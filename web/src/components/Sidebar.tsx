        ...
        return (
          <div key={module.id} className="mb-1.5">
            <button
              ...
              className="w-full flex items-center gap-2 text-left px-2 py-2 rounded-md hover:bg-ink-850 transition-colors"
            >
              <span className={'text-ink-500 text-xs transition-transform ' + (isOpen ? 'rotate-90' : '')}>
                ▶
              </span>
              <span className="text-[13px] font-semibold text-ink-100 leading-tight">{module.title}</span>
            </button>

            {isOpen && (
              <div className="mt-0.5 ml-3 pl-3 border-l border-ink-800 space-y-3 pb-2">
                {module.chapters.map((chapter) => (
                  <div key={chapter.id}>
                    <div className="flex items-center gap-2 px-2 pt-2 pb-1">
                      <span className="text-[11px] font-medium uppercase tracking-wide text-ink-400 leading-tight">
                        {chapter.title}
                      </span>
                      {chapter.comingSoon && (
                        <span className="text-[10px] px-1.5 py-px rounded-full border border-ink-700 text-ink-500">
                          bientôt
                        </span>
                      )}
                    </div>
                        ...
                        >
                          <span
                            className={
                              ...
                              (done ? (active ? 'bg-ink-950' : 'bg-white') : active ? 'bg-ink-400' : 'bg-ink-700')
                            }
                          />
                          <span className="flex-1 leading-tight">{item.title}</span>
                          <span
                            className={
                              'text-[10px] shrink-0 ' + (active ? 'text-ink-600' : 'text-ink-600')
                            }
                          >
                            {itemBadge(item)}
                          </span>
